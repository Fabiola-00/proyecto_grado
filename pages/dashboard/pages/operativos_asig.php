...
try {
    $stmt = $pdo->query("SELECT id, tipo, entidad_responsable, departamento, zona, fecha_inicio, fecha_final, estado FROM operativos ORDER BY fecha_inicio DESC");
    $operativos = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    echo "<p class='error-message'>Error al cargar operativos: " . $e->getMessage() . "</p>";
    $operativos = [];
}
?>
...
<head>
    ...
    <title>SICOSE - Asignación de Operativos</title>
...
<body class="sidebar-expanded">
    ...
    <main id="content" class="content">
        ...
        <div class="form-container">
            <h2>Asignación de Operativos a Instructores</h2>
            ...
            <div class="assignment-form">
                <h3>Asignar Operativo a Instructor</h3>
                <form action="asignar_operativo.php" method="POST">
                    <div class="form-row">
                            ...
                            <select id="operativo_id" name="operativo_id" required>
                                ...
                                <?php foreach ($operativos as $operativo): ?>
                                    <option value="<?= $operativo['id'] ?>">
                                        <?= htmlspecialchars($operativo['tipo'] . ' - ' . $operativo['departamento'] . ' (' . $operativo['estado'] . ')') ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="funcion" name="funcion" required>
                                ...
                                <option value="Personal de rescate">Personal de rescate</option>
                ...
                <tbody>
                    ...
                    try {
                        $sql = "
                            ...
                            ORDER BY i.apellido_paterno ASC, o.fecha_inicio DESC
                        ";
                        ...
                        if (count($asignaciones) > 0) {
                            foreach ($asignaciones as $asig) {
                                ...
                                echo "<td>" . htmlspecialchars($asig['funcion']) . "</td>";
                                echo "<td>" . htmlspecialchars($asig['tipo']) . "</td>";
                                echo "<td>" . htmlspecialchars($asig['entidad_responsable']) . "</td>";
                                echo "<td>" . htmlspecialchars($asig['departamento']) . "</td>";
                                echo "<td>" . htmlspecialchars($asig['zona']) . "</td>";
                                echo "<td>" . htmlspecialchars($asig['estado']) . "</td>";
                                echo "<td>" . date('d/m/Y', strtotime($asig['fecha_inicio'])) . "</td>";
                                echo "<td>" . date('d/m/Y', strtotime($asig['fecha_final'])) . "</td>";
                                echo "<td>
                                        <form method='POST' action='eliminar_asignacion_operativo.php' style='display:inline;'>
                            ...
                            echo "<tr><td colspan='11' style='text-align:center;'>No hay asignaciones registradas</td></tr>";
                        }
                    } catch (PDOException $e) {
                        echo "<tr><td colspan='11' class='error-message'>Error al cargar asignaciones: " . htmlspecialchars($e->getMessage()) . "</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
